...
class WhoisService {
     ...
     */
    findRegistrarRdapUrl(rdap, domain) {
     ...
     */
    async queryRdap(domain, rdapBaseUrl) {
     ...
     */
    formatRdapAsText(domain, rdap, result, registryRdap, registrarRdap) {
        const lines = [];
     ...
     */
    formatContact(lines, prefix, contact) {
        if (contact.name) {
     ...
     */
    parseRdapData(domain, rdap, registryRdap, registrarRdap) {
        const result = {
            domain,
     ...
     */
    parseVcardToContact(vcardArray) {

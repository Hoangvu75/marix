...
const AddServerModal: React.FC<Props> = ({ server, existingTags = [], onSave, onClose }) => {
  ...
  const [data, setData] = useState({
    ...
    password: '',
    icon: 'linux',
    protocol: 'ssh' as 'ssh' | 'js-ssh' | 'ftp' | 'ftps' | 'rdp' | 'wss' | 'mysql' | 'postgresql' | 'mongodb' | 'redis' | 'sqlite',
    ...
    jsScript: '',  // JavaScript script for JS SSH protocol
  });
  ...
  useEffect(() => {
    if (server) {
      setData({
        ...
        protocol: (server as any).jsScript ? 'js-ssh' : (server.protocol || 'ssh'),
        ...
        jsScript: (server as any).jsScript || '',
      });
  ...
  const handleProtocolChange = (protocol: 'ssh' | 'js-ssh' | 'ftp' | 'ftps' | 'rdp' | 'wss' | 'mysql' | 'postgresql' | 'mongodb' | 'redis' | 'sqlite') => {
    let defaultPort = 22;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // JS SSH: only name + script
    if (data.protocol === 'js-ssh') {
      if (!data.name.trim()) {
        alert(t('jsSSHNameRequired') || 'Server name is required');
        return;
      }
      if (!(data as any).jsScript?.trim()) {
        alert(t('jsSSHScriptRequired') || 'Script content is required');
        return;
      }
      const saveData = {
        ...data,
        ...
        username: '',
        password: undefined,
        jsScript: (data as any).jsScript.trim(),
      };
      onSave(saveData);
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-md bg-navy-800 border-l border-navy-700 shadow-2xl flex flex-col animate-in slide-in-from-right duration-200">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div>
            ...
            >
              <optgroup label="Remote Access">
                ...
                <option value="js-ssh">{t('jsSSH') || 'JS SSH'}</option>
          ...
          {/* JS SSH: Script content */}
          {data.protocol === 'js-ssh' && (
            <div>
              <label className="block text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">
                {t('jsSSHScriptContent') || 'JavaScript Script'}
              </label>
              <textarea
                value={(data as any).jsScript || ''}
                onChange={(e) => setData(prev => ({ ...prev, jsScript: e.target.value }))}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">
              {data.protocol === 'wss' ? t('displayName') : data.protocol === 'js-ssh' ? t('serverName') : t('hostName')}
            </label>
            <input
              type="text"
              name="name"
          ...
          {data.protocol !== 'wss' && data.protocol !== 'js-ssh' && (
          ...
          {/* Username (not for WSS, not for JS SSH) */}
          {data.protocol !== 'wss' && data.protocol !== 'js-ssh' && (
            <div>
          ...
          {/* Password field (not for WSS, not for JS SSH) */}
          {(data.authType === 'password' || data.protocol !== 'ssh') && data.protocol !== 'wss' && data.protocol !== 'js-ssh' && (
            <div>

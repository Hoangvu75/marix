...
const CommandRecallPopup: React.FC<Props> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20">
      ...
      {/* Popup */}
      <div 
        className={`relative w-full max-w-xl mx-4 rounded-lg shadow-2xl overflow-hidden ${
          isDark ? 'bg-navy-800 border border-navy-600' : 'bg-white border border-gray-200'
        }`}
      ...
      >
        ...
        <div className={`p-3 border-b ${isDark ? 'border-navy-600' : 'border-gray-200'}`}>
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder={(t as any).commandRecallSearch || "Search command history..."}
              className={`flex-1 bg-transparent outline-none text-sm ${
                isDark ? 'text-white placeholder-gray-500' : 'text-gray-900 placeholder-gray-400'
              }`}
            />
            <kbd className={`px-2 py-0.5 text-xs rounded ${
              isDark ? 'bg-navy-700 text-gray-400' : 'bg-gray-100 text-gray-500'
            }`}>
              Esc
            </kbd>
          </div>
          ...
          {filteredCommands.length === 0 ? (
            <div className={`p-8 text-center ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
              {commands.length === 0 ? (
                <>
                  ...
                  <p className="text-sm">{(t as any).commandRecallEmpty || "No command history yet"}</p>
                  <p className="text-xs mt-1 opacity-70">{(t as any).commandRecallEmptyHint || "Commands will appear here as you use the terminal"}</p>
                </>
              ) : (
                <p className="text-sm">{(t as any).commandRecallNoMatch || "No matching commands"}</p>
              )}
            </div>
          ) : (
            filteredCommands.map((cmd, index) => (
              <div
                ...
                className={`group px-3 py-2 cursor-pointer flex items-center gap-3 ${
                  index === selectedIndex
              ...
              >
                ...
                <div className="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    ...
                    title={(t as any).commandRecallSaveSnippet || "Save as Snippet"}
                  >
        ...
        <div className={`px-3 py-2 border-t text-xs flex items-center justify-between ${
          ...
          <span>{filteredCommands.length} {(t as any).commands || "commands"}</span>
        </div>
      </div>
    </div>
  );
